...
import { Target, Zap, PlusCircle, Medal, RefreshCw } from "lucide-react";
    ...
    <div className="flex-1 w-full bg-[#f9f9f7] relative z-0 min-h-[calc(100vh-80px)] overflow-y-auto">
      <main className="px-4 md:px-margin-desktop py-8 md:py-12 w-full max-w-max-width-content mx-auto">
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={handleSync}
              disabled={isSyncing}
              className="pop-card bg-xp-gold px-6 py-2 font-bold uppercase tracking-wider flex items-center gap-2 disabled:opacity-60 transition-opacity"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-8 space-y-6">
            ...
            {loading ? (
              ...
              <div className="p-6 text-center italic text-on-surface-variant animate-fade-in-up">
                No active sprints. Time to set one!
              </div>
            ) : (
              goals.map((goal, index) => (
                <div
                  key={goal.id}
                  className="pop-card bg-white p-6 relative overflow-hidden animate-fade-in-up opacity-0"
                ...
                >
                  <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4 gap-4">
                    ...
                    <div className="bg-xp-gold px-3 py-1 rounded-full border-2 border-on-surface font-label-xp text-label-xp flex items-center justify-center gap-1 shrink-0 whitespace-nowrap uppercase">
                      {goal.status || "ACTIVE"}
                    </div>
                  </div>
          ...
          <div className="md:col-span-4 space-y-6">
            ...
            <div className="space-y-4">
              {loading ? (
                <div className="space-y-4">
                  {[0, 1, 2].map((i) => (
                    <div key={i} className="pop-card bg-white p-4 h-[100px] flex items-center justify-center">
                ...
                habits.map((habit, index) => (
                  <div
                    key={habit.id}
                    className="pop-card bg-white p-4 animate-fade-in-up opacity-0"
                    style={{ animationDelay: `${index * 50}ms` }}
                  >
                    ...
                    {/* Simple streak progress bar instead of fake dots */}
                    <div className="h-2 bg-surface-container-high rounded-full overflow-hidden border border-on-surface/20">
                      <div
                        className="h-full bg-xp-gold transition-all duration-500"
                        style={{ width: `${Math.min(100, ((habit.current_streak || 0) / 30) * 100)}%` }}
                      />
                    </div>
                    <p className="text-[10px] text-on-surface-variant font-bold mt-1 uppercase">
                      {habit.frequency} • {habit.current_streak || 0} day streak
                    </p>
                  </div>
                ))
              )}
              ...
              <div className="pop-card bg-on-surface text-white p-6 relative overflow-hidden mt-6">
                <div className="absolute -right-4 -bottom-4 halftone opacity-40 rotate-12 w-24 h-24" />
                ...
                <p className="text-sm opacity-80 mb-4 relative z-10">
                  Knock out your quests and log your habits daily to unlock XP multipliers and level up faster.
                </p>
                <a href="/chat" className="block w-full bg-white text-on-surface py-2 font-bold rounded relative z-10 hover:bg-xp-gold transition-colors text-center text-sm">
                  Open Chat
                </a>
              </div>

              {/* Stats sticker — real data */}
              {!loading && goals.length > 0 && (
                <div className="pop-card bg-[#FF6B6B] p-4 rotate-2 mt-6 max-w-[200px] ml-auto">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] uppercase font-bold text-white/80">Active Sprints</p>
                      <p className="text-xl font-extrabold text-white anton-text tracking-wide">{goals.length} GOAL{goals.length !== 1 ? "S" : ""}</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="md:hidden mt-8 sticky bottom-4">
          <button
            onClick={() => setIsModalOpen(true)}
            className="w-full pop-card bg-indigo-deep text-white py-4 font-bold flex items-center justify-center gap-3"
          >
            <Zap size={20} />
            NEW SPRINT
          </button>
